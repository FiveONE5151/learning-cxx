...
// 通常来说，形状为 [d0, d1, ..., dn] 的张量，第 n 维是 dn 度指的是张量每个维度上坐标 +1 时，数据指针跨过的个连续的元素，第 n-1 维是 dn-1 个连续的 dn 个元素，以此类推。
// 张量的步长或跨范围。
...
std::vector<udim> strides(std::vector<udim> const &shape) {
    ...
    //       使用逆向迭代器可能可以简化代码
    auto pShape = shape.rbegin();
    auto pStride = strides.rbegin();
    *pStride++ = 1;
    for (; pShape != shape.rend() - 1; pShape++, pStride++) {
        *pStride = *(pStride - 1) * (*pShape);
    }
